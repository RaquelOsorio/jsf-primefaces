...
@Stateless
public class DetalleCompraRegistration {
    ...
    @Inject
    private Event<DetalleCompra> compra_detEventSrc;


    public void register(DetalleCompra producto) throws Exception {
        log.info("Registering " + producto.getProducto());//poner get producto
        em.persist(producto);
        compra_detEventSrc.fire(producto);
    }
    public void modificar(DetalleCompra detalle) throws Exception {
        log.info("Registering " + detalle.getId());
        em.merge(detalle);
        compra_detEventSrc.fire(detalle);
    }
    public void remover(DetalleCompra detalle) throws Exception {
