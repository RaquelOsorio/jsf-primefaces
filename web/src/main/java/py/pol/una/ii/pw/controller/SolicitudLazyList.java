...
@ManagedBean
public class SolicitudLazyList extends LazyDataModel<SolicitudCompra>{
    ...
    @Override
    public SolicitudCompra getRowData(String rowKey) {
        String output="";
        try {
            ClientRequest request = new ClientRequest("http://localhost:8080/EjbJaxRS-web/rest/solicitudes/" + rowKey);
        ...
        return gson.fromJson(output, SolicitudCompra.class);
    }
    
    @Override
    public Object getRowKey(SolicitudCompra cliente) {
    ...
    {
        return SolicitudLazyList.filtrado;
    }
    
    public void setFiltrado(Map<String,Object> fil)
    {
    	SolicitudLazyList .filtrado = fil;
    }
    
    @Override
    public List<SolicitudCompra> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters){
        ...
        try {
           request = new ClientRequest("http://localhost:8080/EjbJaxRS-web/rest/solicitudes/filtrarCantidad/" + stringFiltros);
           request.accept("application/json");
        ...
        try {
            request = new ClientRequest("http://localhost:8080/EjbJaxRS-web/rest/solicitudes/filtrar/" + stringFiltros);
            request.accept("application/json");
            response = request.get(String.class);
            br = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(response.getEntity().getBytes())));
            output = br.readLine();
        	...
        	solicitudes = gson.fromJson(output, new TypeToken<List<SolicitudCompra>>(){}.getType());
            return solicitudes;
        }
}
